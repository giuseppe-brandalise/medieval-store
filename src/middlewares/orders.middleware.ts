...
const verifyProductsIds = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const checkArray = productIds.every((id: number) => typeof id === 'number');
  if (!checkArray || productIds.length === 0) {
